...
-- Настройки магнита
local RADIUS = 25 -- Радиус притяжения, можешь менять
local SPEED = 0.15 -- Скорость притяжения

-- Функция для создания невидимого пола
local function toggleInvisibleFloor()
    if invisibleFloor then
        -- Удаляем невидимый пол
        ...
        -- Создаем невидимый пол под ногами игрока
...
-- Функция для активации магнита
local function toggleMagnet()
    magnetActive = not magnetActive
end

-- Создаем цикл для магнита
RunService.RenderStepped:Connect(function()
    if magnetActive then
        for _, part in ipairs(workspace:GetDescendants()) do
            if part:IsA("BasePart") and not part.Anchored and part ~= humanoidRootPart then
                local distance = (part.Position - humanoidRootPart.Position).Magnitude
                if distance <= RADIUS then
                    -- Притягиваем часть
                    local direction = (humanoidRootPart.Position - part.Position).Unit
                    part.Velocity = direction * (distance * SPEED)

                    -- Добавляем немного вращения
                    local rotVelocity = Vector3.new(
                        math.random(-2, 2),
                        math.random(-2, 2),
                        math.random(-2, 2)
                    )
                    part.RotVelocity = rotVelocity
                end
            end
        end
    end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.Y then
        toggleInvisibleFloor() -- Включение/выключение невидимого пола
    elseif input.KeyCode == Enum.KeyCode.M then
        toggleMagnet() -- Включение/выключение магнита
    end
end)
